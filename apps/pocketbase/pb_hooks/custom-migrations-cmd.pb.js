...
function resolvePocketBaseExecutable() {
    const unixPath = $filepath.join(__hooks, "../pocketbase")
    const windowsPath = $filepath.join(__hooks, "../pocketbase.exe")
...
const pocketBaseExecutable = resolvePocketBaseExecutable()
...
horizonsCmd.addCommand(new Command({
    ...
    run: (cmd, _args) => {
        ...
        const result = $os.cmd(pocketBaseExecutable, ...cmdArgs)
